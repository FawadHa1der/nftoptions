...
async function fetcher(key: string, owner: string, contractAddress: string, tokenId: string) {
  ...
  try {
    ...
    return approvedAddress.toString(16) != '0' && OPTIONS_CONTRACT_ADDRESS.includes(approvedAddress.toString(16))
  } catch {
    return false
  }
}
...
export default function useIsERC721Approved(contractAddress: string, tokenId: string): boolean {
  ...
  const { data } = useSWR(owner ? ['IsERC721Approved', owner, contractAddress, tokenId] : null, fetcher)
  return data ?? false
}
