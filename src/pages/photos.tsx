...
export default function Photos() {
  ...
  async function onRegistered(optionData: IPutOptionForm) {
    ...
    if (formType == PutOptionFormType.CREATE) {
      ...
      let myAddress = getStarknet().account.address
      ...
      const allowanceResult = await callContract(erc20ContractInstance, 'allowance', getStarknet().account.address, optionsAddress)
      ...
      if (existingMoneyAllowance < parseInt(optionData.premium)) {
        await sendTransaction(erc20ContractInstance, 'approve', { spender: optionsAddress, amount: getUint256CalldataFromBN(100000000) })
      }

      toast({ description: 'Registering your bid now' });
      let transaction_response = await sendTransaction(nftOptionsContractInstance, 'register_put_bid', paramStruct)
      ...
      toast({ description: 'Selling the put option' });
      let transaction_response = await sendTransaction(nftOptionsContractInstance, 'exercise_put', { bid_id: putData?.bid_id })
      toast({ description: 'The option is active now, data will take a few mins to reflect in the UI' });
    }
  }
